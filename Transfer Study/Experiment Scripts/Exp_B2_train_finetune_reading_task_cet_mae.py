...
def parse_args():
    parser = argparse.ArgumentParser(
        description="B2: Fine-tuned reading-task classification for CET-MAE "
        "(ZuCo 2.0 NR vs TSR, LOSO by subject)"
    )
    ...
    parser.add_argument(
        "--random-init",
        action="store_true",
        help="Use random encoder weights instead of loading a checkpoint.",
    )
    parser.add_argument("--output-dir", type=str, required=True)

    # FIXED: default 25 to match plan / B1
    ...
    # Keep default behavior = True, but make disabling possible
    parser.add_argument(
        "--no-class-weights",
...
def setup_logging(output_dir: Path):
    ...
    logger = logging.getLogger("b2_finetune_reading_zuco2")
...
def combine_nr_tsr(npz_nr, npz_tsr, logger):
    ...
    if npz_nr["eeg"].ndim != 3 or npz_tsr["eeg"].ndim != 3:
        raise ValueError(
            "B2 fine-tuning requires raw token-level EEG arrays of shape (N, L, D) for both NR and TSR. "
            f"Found NR={npz_nr['eeg'].shape}, TSR={npz_tsr['eeg'].shape}. "
            "At least one NPZ appears to contain pooled sentence embeddings instead of token-level EEG features."
        )
    ...
    logger.info(
        f"Subjects in final dataset: {sorted(meta['subject'].astype(str).unique().tolist())}"
    )
...
class FinetuneReadingTaskModel(nn.Module):
    def __init__(
        self,
        checkpoint_path,
        bart_path,
        ...
        if random_init:
            ...
            if checkpoint_path is None:
                raise ValueError(
                    "checkpoint_path must be provided unless --random-init is set."
                )
    ...
    def encode(self, eeg, attention_mask):
        if eeg.ndim != 3:
            raise ValueError(
                f"Expected EEG input of shape (N, L, D), got {tuple(eeg.shape)}"
            )
    ...
    def encoder_parameters(self):
        modules = [self.pos_embed_e, self.e_branch, self.fc_eeg]
        if self.unify_branch is not None:
            modules.append(self.unify_branch)
        for module in modules:
            yield from module.parameters()


# ---------------------------------------------------------------------
# Dataset / metrics / train loop
# ---------------------------------------------------------------------
...
def make_class_weights(y_train, num_classes, device):
...
def split_train_val_subjects(trainval_subjects, val_ratio, seed):
    ...
    if len(trainval_subjects) < 2:
        raise ValueError(
            "Need at least 2 train/val subjects after holding one out for test."
        )
...
def run_epoch(model, loader, optimizer, device, train, criterion):
...
def evaluate_split(model, loader, device, criterion):
    metrics, y_true, y_pred, prob = run_epoch(
        model, loader, optimizer=None, device=device, train=False, criterion=criterion
    )
    return {
        "metrics": metrics,
        "y_true": y_true,
...
def save_pca_plots(features, meta_df, out_prefix, color_columns):
    ...
    for col in color_columns:
        ...
        ax.set_xlabel("PCA-1")
        ax.set_ylabel("PCA-2")
        ax.set_title(f"PCA of encoded embeddings colored by {col}")
        ax.grid(alpha=0.2)
        if len(vals.unique()) <= 20:
...
def save_eval_artifacts(eval_out, meta_subset, class_names, split_dir):
    ...
    pred_df["true_label_name"] = pred_df["true_label_id"].map(
        {0: class_names[0], 1: class_names[1]}
    )
    pred_df["pred_label_name"] = pred_df["pred_label_id"].map(
        {0: class_names[0], 1: class_names[1]}
    )
    ...
    plot_confusion(
        ...
        f"Confusion matrix ({split_dir.name})",
    )
...
def main():
    ...
    if (not args.random_init) and (args.checkpoint is None):
        raise ValueError(
            "Provide --checkpoint for pretrained initialization, or pass --random-init."
        )
    ...
    logger.info("Starting B2 fine-tuned reading-task classification (ZuCo 2.0 NR vs TSR)")
    ...
    f"class_weights_enabled={args.use_class_weights}."
    )
    ...
    logger.info(
        f"Running leave-one-subject-out CV over {len(subjects_unique)} subjects."
    )
    ...
    for fold_idx, held_out_subject in enumerate(subjects_unique, start=1):
        ...
        fold_meta = meta.copy()
        fold_meta["split"] = np.where(
            test_mask, "test", np.where(val_mask, "val", "train")
        )
        ...
        eeg_val = eeg[val_mask]
        mask_val = mask[val_mask]
        ...
        logger.info(
            f"Train size: {len(eeg_train)} | Val size: {len(eeg_val)} | "
            f"Test size: {len(eeg_test)}"
        )
        logger.info(
        ...
        if args.use_class_weights:
            class_weights = make_class_weights(y_train, num_classes=2, device=device)
            logger.info(f"Class weights: NR={class_weights[0]:.4f}, TSR={class_weights[1]:.4f}")
            criterion = nn.CrossEntropyLoss(weight=class_weights)
        else:
            criterion = nn.CrossEntropyLoss()

        model = FinetuneReadingTaskModel(
        ...
        for epoch in range(1, args.epochs + 1):
            train_metrics, *_ = run_epoch(
                model, train_loader, optimizer, device, train=True, criterion=criterion
            )
        ...
        torch.save(model.state_dict(), fold_dir / "last_finetuned_model.pt")
        ...
        plot_training_curves(
            ...
            f"B2 Fine-tuned - Held-out Subject {held_out_subject} (best epoch {best_epoch})",
        )
        ...
        for split_name, loader, mask_now in [
            ...
            eval_out = evaluate_split(model, loader, device, criterion)
            meta_subset = fold_meta.loc[mask_now].copy().reset_index(drop=True)
            save_eval_artifacts(eval_out, meta_subset, class_names, fold_dir / split_name)

            if split_name == "test":
                cm = confusion_matrix(
                    eval_out["y_true"], eval_out["y_pred"], labels=[0, 1]
                )
                aggregate_cm += cm
    ...
    logger.info("=" * 88)
    logger.info("Finished B2 fine-tuned reading-task classification (ZuCo 2.0 NR vs TSR)")
