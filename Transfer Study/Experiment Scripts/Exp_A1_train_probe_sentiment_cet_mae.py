...
try:
    ...
    raise ImportError(
        ...
        "Please verify that transfer_study/model_mae_bart.py defines Pooler."
    ) from e
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="A1: Frozen sentiment probe for CET-MAE (LOSO by subject, trial-level)"
    )
    ...
    parser.add_argument(
        "--binary",
        action="store_true",
        help="If set, drop neutral and run binary pos/neg classification.",
    )
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="NPZ contains token-level hidden states (N,L,1024); pool them with model_mae_bart.Pooler",
    )

    args = parser.parse_args()
    if not args.freeze_encoder:
        parser.error("A1 is a frozen-probe experiment; pass --freeze-encoder to run this script.")
    return args


def setup_logging(output_dir: Path):
    output_dir.mkdir(parents=True, exist_ok=True)
    logger = logging.getLogger("a1_frozen_probe")
...
def load_npz_data(npz_path, logger):
    ...
    invert_mask_key = first_existing_key(
        keys,
        [
...
def build_merged_table(npz_data, labels_df, logger, binary=False):
    ...
    logger.info(
        f"Label counts: {merged['sentiment_raw'].value_counts().to_dict()}"
    )
...
def extract_features(npz_data, args, device, output_dir, logger):
    ...
    if args.input_is_pooled or (eeg.ndim == 2 and eeg.shape[1] == 1024):
        ...
        feats = Pooler(
            eeg.astype(np.float32), mask.astype(np.float32)
        ).astype(np.float32)
...
class LinearProbe(nn.Module):
    def __init__(self, in_dim=1024, num_classes=3):
        super().__init__()
...
def main():
    ...
    logger.info("Starting A1 frozen sentiment probe (trial-level, text-joined)")
    ...
    merged, class_names = build_merged_table(
        npz_data, labels_df, logger, binary=args.binary
    )
    ...
    for fold_idx, held_out_subject in enumerate(subjects_unique, start=1):
        ...
        train_subjects, val_subjects = split_train_val_subjects(
            np.unique(subj_trainval).tolist(), args.val_ratio, args.seed + fold_idx
        )
        train_idx = np.isin(subj_trainval, train_subjects)
        val_idx = np.isin(subj_trainval, val_subjects)
        ...
        logger.info(
            f"Train subjects: {train_subjects} | Val subjects: {val_subjects} | Test subject: {held_out_subject}"
        )
        logger.info(
        ...
        plot_training_curves(
            ...
            f"A1 Frozen Probe - Subject {held_out_subject} (best epoch {best_epoch})",
        )
    ...
    logger.info("=" * 88)
    logger.info("Finished A1 frozen sentiment probe")
